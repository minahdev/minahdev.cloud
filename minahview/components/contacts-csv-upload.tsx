...
export function ContactsCsvUpload({ onUploaded }: { onUploaded?: (saved: number) => void }) {
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      const json = ((): { saved?: number; error?: string; detail?: string } => {
        try {
      ...
      const count = typeof json.saved === "number" ? json.saved : null
      setUploadedCount(count)
      if (count != null) onUploaded?.(count)
    } catch {
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div
        ...
        onDrop={onDrop}
        className={cn(
          "rounded-xl border-2 border-dashed px-5 py-8 text-center transition-colors outline-none",
          isDragging
      ...
      >
        ...
        <p className="text-xs text-muted-foreground mb-6">헤더: 닉네임, 이메일 (드래그 앤 드롭 가능)</p>
      ...
      {file ? (
        <div className="rounded-lg border border-border bg-secondary/50 px-4 py-3 text-sm">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
            ...
            {uploadedCount != null ? (
              <p className="text-xs text-muted-foreground">총 {uploadedCount.toLocaleString()}건 저장됨</p>
            ) : null}
          </div>
        </div>
      ) : null}
