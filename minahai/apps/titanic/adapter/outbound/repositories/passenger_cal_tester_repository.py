...
class CalTesterRepository(CalTesterPort):
    ...
    async def introduce_myself(self, query: CalTesterQuery) -> CalTesterResponse:
        logger.info(f"[CalTesterPgRepository] introduce_myself | request_data={query}")
    ...
    async def get_training_data(self) -> TrainingData:
        ...
        for passenger, booking in rows:
            ...
            features = [
                ...
                1.0 if passenger.gender == "male" else 0.0,
        ...
        logger.info(f"[CalTesterPgRepository] get_training_data | rows={len(y)}")
